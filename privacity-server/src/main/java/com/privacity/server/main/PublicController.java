...
//	private int interationCount;
	...
	public PublicController(FacadeComponent comps) throws Exception {
		...
		this.comps = comps;
		getMapaController().put(ConstantProtocolo.PROTOCOLO_COMPONENT_AUTH, comps.validation().auth());
		getMapaMetodos().put(ConstantProtocolo.PROTOCOLO_ACTION_AUTH_LOGIN, comps.validation().auth().getClass().getMethod(AuthValidationService.METHOD_ACTION_AUTH_LOGIN, LoginRequestDTO.class));
		getMapaMetodos().put(ConstantProtocolo.PROTOCOLO_ACTION_AUTH_REGISTER, comps.validation().auth().getClass().getMethod(AuthValidationService.METHOD_ACTION_AUTH_REGISTER, RegisterUserRequestDTO.class));
		getMapaMetodos().put(ConstantProtocolo.PROTOCOLO_ACTION_AUTH_VALIDATE_USERNAME, comps.validation().auth().getClass().getMethod(AuthValidationService.METHOD_ACTION_AUTH_VALIDATE_USERNAME, ValidateUsernameDTO.class));

		getMapaController().put(ConstantProtocolo.PROTOCOLO_COMPONENT_REQUEST_ID, comps.validation().requestId());
		getMapaMetodos().put(ConstantProtocolo.PROTOCOLO_ACTION_REQUEST_ID_PUBLIC_GET, comps.validation().requestId().getClass().getMethod("getNewRequestIdPublic", RequestIdDTO.class));		
